...
function createInstance(instance) {
  ...
  const spot = isSpot(instance)
  const emr = isEmr(instance)
  return {
    family,
    size,
    az,
  ...
  loadInstances(region) {
    return this._cacheResult('instances', region, () => {
      return this._ec2Client(region)
        ...
        .promise()
        .then((response) => response.Reservations.reduce((all, reservation) => all.concat(reservation.Instances.map(createInstance)), []))
    })
  }
