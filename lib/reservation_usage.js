...
class ApiGatewayFormatter {
  ...
  _acceptsJson() {
    return /application\/json/.test(this._event.headers.Accept)
  }
...
class SlackFormatter extends ApiGatewayFormatter {
  ...
  formatResponse(summary) {
    const response = super.formatResponse(summary)
    response.body = '```\n' + response.body + '```\n'
    return response
  }
...
module.exports = class ReservationUsage {
  ...
  _createAuthenticator(event) {
    if (this._isApiGatewayEvent(event) && this._isSlackEvent(event)) {
  ...
  _createResponseFormatter(event) {
    if (this._isApiGatewayEvent(event)) {
      if (this._isSlackEvent(event)) {
        return new SlackFormatter(event)
      } else {
  ...
  _matchReservations(reservations, instances) {
    ...
    const spotInstances = []
    const emrInstances = []
    const onDemandInstances = []
    const unreservedInstances = []
    instances.forEach((instance) => {
      if (instance.spot) {
        spotInstances.push(instance)
      } else {
        ...
        if (instance.emr) {
          emrInstances.push(instance)
        } else {
          onDemandInstances.push(instance)
        }
      }
    })
  ...
  _summarizeFindings(onDemand, spot, emr, unreserved, reservations, unusedReservations) {
    const summaries = {
      onDemand: this._summarizeByFamily(onDemand, 'units'),
      spot: this._summarizeByFamily(spot, 'units'),
      emr: this._summarizeByFamily(emr, 'units'),
      reserved: this._summarizeByFamily(reservations, 'units'),
      unreserved: this._summarizeByFamily(unreserved, 'units'),
